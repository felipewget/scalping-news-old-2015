<?php 
	...
	*/
	...
	$class = new PortalJokenpo();

	$pagina = $class->acessLink('http://portaljokenpo.com.br/');
	...
	foreach($arrBox as $blocoUn)	
	{
		...
		$img = strip_tags($class->cortaCodigo($blocoUn, '<div class="post-thumbnail">', '</div>'));
		$img = strip_tags($class->cortaCodigo($blocoUn, 'src="', '"'));

		// $data = implode("-", array_reverse(explode("/", ($class->cortaCodigo($blocoUn, '<div class="data">', '</div>')))));

		if(strlen($link) > 5 && strlen($titulo) > 5){

			// echo $img = str_replace('-310x165', '', $img);
			// echo '<br />';
			// echo '<hr />';
			//  echo $titulo;
			//  echo '<br/>';
			...
			// echo '<br/>';
			 ...
			 if(strlen(strstr($link, 'portaljokenpo')) > 5){

				$paginaLinkProvisorio = $class->acessLink($link);
				
				$conteudo = $class->cortaCodigo($paginaLinkProvisorio, '<div class="entry">', '<!-- .entry /-->');
				$arrConteudo = explode('<div id="vc-feelback-main"', $conteudo);
				$conteudo = $arrConteudo[0];
